...
|Decentralization| **Additional Collateral:** Nerite accepts: ETH, wstETH, rETH, rsETH, weETH, ARB, COMP, tBTC|
...
Unlike most DeFi protocols, Nerite has **no separate DAO, foundation, operational company, or labs entity**. There is no Nerite Foundation, no Nerite Labs, no operating company extracting value. It's just NERI — the token itself is the entire governance and value-flow structure.
